"""
GLM-5 FlashDSA reconstructed modeling file.
...
class Glm5ReconstructedConfig(PreTrainedConfig):
    ...
    def __init__(
        ...
        mla_qk_dim: int = 576,
        mla_v_dim: int = 512,
        flashdsa_qk_dim: int = 192,
        flashdsa_v_dim: int = 256,
        flashdsa_kv_lora_dim: int = 512,
        flashdsa_indexer_heads: int = 32,
        flashdsa_indexer_head_dim: int = 128,
        ...
        **kwargs,
        ...
        self.mla_qk_dim = mla_qk_dim
        self.mla_v_dim = mla_v_dim
        self.flashdsa_qk_dim = flashdsa_qk_dim
        self.flashdsa_v_dim = flashdsa_v_dim
        self.flashdsa_kv_lora_dim = flashdsa_kv_lora_dim
        self.flashdsa_indexer_heads = flashdsa_indexer_heads
        self.flashdsa_indexer_head_dim = flashdsa_indexer_head_dim
    ...
    def head_dim(self) -> int:
        if self.attention_backend == "flash_dsa":
            return self.flashdsa_qk_dim
        if self.use_latent_kv:
            return self.mla_qk_dim
        return self.hidden_size // self.num_attention_heads

    @property
    def value_head_dim(self) -> int:
        if self.attention_backend == "flash_dsa":
            return self.flashdsa_v_dim
        if self.use_latent_kv:
            return self.mla_v_dim
        return self.head_dim

    @property
    def kv_lora_dim(self) -> int:
        if self.attention_backend == "flash_dsa":
            return self.flashdsa_kv_lora_dim
        return self.head_dim + self.value_head_dim
...
class SparseDSAAttention(nn.Module):
    ...
    def __init__(self, config: Glm5ReconstructedConfig):
        ...
        if self.use_latent_kv:
            self.kv_proj = nn.Linear(
                config.hidden_size,
                config.num_key_value_heads * self.kv_lora_dim,
                bias=False,
            )
            self.k_proj = None
            self.v_proj = None
            ...
            self.v_proj = nn.Linear(config.hidden_size, config.num_key_value_heads * self.value_head_dim, bias=False)
        self.o_proj = nn.Linear(config.num_attention_heads * self.value_head_dim, config.hidden_size, bias=False)
        ...
        if self.attention_backend == "flash_dsa":
            self.indexer_q_proj = nn.Linear(
                config.hidden_size,
                config.flashdsa_indexer_heads * config.flashdsa_indexer_head_dim,
                bias=False,
            )
            self.indexer_k_proj = nn.Linear(
                config.hidden_size,
                config.num_key_value_heads * config.flashdsa_indexer_head_dim,
                bias=False,
            )
        else:
            self.indexer_q_proj = None
            self.indexer_k_proj = None
    ...
    def _can_use_flash_mla_prefill(
        ...
        if self.num_kv_heads != 1:
            ...
            return False
        if self.head_dim != 576:
            self._warn_flash_fallback(f"FlashMLA sparse prefill expects d_qk=576, got {self.head_dim}")
            return False
        if self.value_head_dim != 512:
            self._warn_flash_fallback(f"FlashMLA sparse prefill expects d_v=512, got {self.value_head_dim}")
            return False
    ...
    def _build_flash_mla_indices(
        ...
        topk = self.config.dsa_topk
        valid_topk = min(topk, seed_scores.shape[-1])
        top_idx = seed_scores.topk(k=valid_topk, dim=-1).indices[0, 0]
        indices = torch.full(
        ...
        indices[:, 0, :valid_topk] = top_idx.to(torch.int32)
        return indices
    ...
    def _flash_mla_prefill(
        ...
        indices = self._build_flash_mla_indices(q, k_latent, attention_mask)
        q_fm = q[0].transpose(0, 1).contiguous()
        kv_fm = k_latent[0].transpose(0, 1).contiguous()
    ...
    def forward(
        ...
        if self.use_latent_kv:
            kv_latent = self.kv_proj(hidden_states).view(
                bsz, seqlen, self.num_kv_heads, self.kv_lora_dim
            ).transpose(1, 2)
            if self.attention_backend == "flash_dsa":
                qk_latent, v_latent, index_latent = torch.split(
                    kv_latent,
                    [self.head_dim, self.value_head_dim, self.kv_lora_dim - self.head_dim - self.value_head_dim],
                    dim=-1,
                )
                k_latent = qk_latent
                k = qk_latent
                ...
                index_q = self.indexer_q_proj(hidden_states).view(
                    bsz, seqlen, self.config.flashdsa_indexer_heads, self.config.flashdsa_indexer_head_dim
                ).transpose(1, 2)
                index_k = self.indexer_k_proj(hidden_states).view(
                    bsz, seqlen, self.num_kv_heads, self.config.flashdsa_indexer_head_dim
                ).transpose(1, 2)
            else:
                index_q = None
                index_k = None
                k_latent, v_latent = torch.split(kv_latent, [self.head_dim, self.value_head_dim], dim=-1)
                k = k_latent
                v = v_latent
        else:
            kv_latent = None
            ...
            v = self.v_proj(hidden_states).view(bsz, seqlen, self.num_kv_heads, self.value_head_dim).transpose(1, 2)
        ...
        if self._can_use_flash_mla_prefill(q, k_latent, past_key_value, use_cache):
            flash_out, flash_attn = self._flash_mla_prefill(q, k_latent, attention_mask)
            return flash_out, flash_attn, present_key_value
        if self._can_use_flash_dsa(q, k_latent, past_key_value, use_cache):
            flashdsa_out, flashdsa_attn = self._flashdsa_sparse_attention(q, index_q, index_k, kv_latent, attention_mask)
            return flashdsa_out, flashdsa_attn, present_key_value
